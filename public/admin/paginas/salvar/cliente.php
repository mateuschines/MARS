		...
		if ( empty ($id) ) {
			...
			if ($clienteC->inserirCliente($nome, $cpf, $rg, $endereco, $celular, $email, $senha, $whatsapp, $sexo, $dtNascimento, $cep, $complemento, $bairro, $numero, $cidade_id)) {
				if($_SESSION["sistema"]["tipo"] == "Master") {
					echo "<script>alert('Registro salvo');location.href='home.php?op=listas&pg=cliente';</script>";
				  } else {
					echo "<script>alert('Registro salvo');location.href='homeA.php?op=listas&pg=cliente';</script>";
				  }
			...
			if ( empty($senha) ) {
				...
				if ($clienteC->editarEmpCliente($id, $nome, $cpf, $rg, $endereco, $celular, $email, $whatsapp, $sexo, $dtNascimento, $cep, $complemento, $bairro, $numero, $cidade_id)) {
					...
					if($_SESSION["sistema"]["tipo"] == "Master") {
						echo "<script>alert('Registro atualizado');location.href='home.php?op=listas&pg=cliente';</script>";
						exit;
					  } else {
						echo "<script>alert('Registro atualizado');location.href='homeA.php?op=listas&pg=cliente';</script>";
						exit;
					  }
					...
					if ($clienteC->editarCliente($id, $nome, $cpf, $rg, $endereco, $celular, $email, $senha, $whatsapp, $sexo, $dtNascimento, $cep, $complemento, $bairro, $numero, $cidade_id)) {
						...
						if($_SESSION["sistema"]["tipo"] == "Master") {
							echo "<script>alert('Registro atualizado com senha');location.href='home.php?op=listas&pg=cliente';</script>";
							exit;
						  } else {
							echo "<script>alert('Registro atualizado com senha');location.href='homeA.php?op=listas&pg=cliente';</script>";
							exit;
						  }
